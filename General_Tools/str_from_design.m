...
% Creates a string based on a design which can input properties of the struct_in
% design: [property-name], / or \ will call filesep (SEE: DB_Class.file_path.m)
%
% EXAMPLE:
%     the path to data is defined by struct_in.subject(NS01) and struct_in.session(02)
%     NS01/S02 --> '[subject]/S[session]'
%
% 2013-10-04 Foldes
% UPDATES:
%
...
while char_cnt < length(design_str)
    ...
    switch current_char
        case '[' % start of a struct_in.property
            ...
            eval(['str_out = [str_out ''' struct_in.(prop_name) '''];'])
