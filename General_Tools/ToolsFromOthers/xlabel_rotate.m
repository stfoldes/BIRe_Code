function hh = xlabel_rotate(rot_angle,ax)
% Examples:
%   xlabel_rotate [rotates current x-label (gca) by 45 degrees]
%   xlabel_rotate(90) [rotates current x-label (gca) by 90 degrees]
%
% Based on:rotateXLabels.m from Matlab Central
% Can see a simpler version that looks the same: rotateticklabel.m
%
% Changed: Foldes 2013-07-11
% Is now a standalone command that can just be used straight up
% Assumes axis is the gca
% 
...
if ~exist('rot_angle') || isempty(rot_angle)
    rot_angle = 45;
end


if ~isnumeric( rot_angle ) || ~isscalar( rot_angle )
    error( 'RotateXLabels:BadAngle', 'Parameter rot_angle must be a scalar angle in degrees' )
end
rot_angle = mod( rot_angle, 360 );
...
h = createNewLabels( ax, vals, labels, rot_angle );
...
% Store rot_angle
setappdata( ax, 'RotateXLabelsAngle', rot_angle );
...
%-------------------------------------------------------------------------%
    function textLabels = createNewLabels( ax, vals, labels, rot_angle )
...
%-------------------------------------------------------------------------%
    ...
    function updateAlignment()
        textLabels = findall( ax, 'Tag', 'RotatedXTickLabel' );
        rot_angle = get( textLabels(1), 'Rotation' );
