function Copy_List = dependency_pkg(SRC, DEST, IS_recursive)
...
%   dependency_pkg('EXAMPLE_Mark_Bad_MEG.m')
%   dependency_pkg; % guis
% 
% 2011/04/20 Jit Sarkar
% UPDATES:
% 2013-10-11 Foldes: Comment and cleaned, UI and smarts added, renamed from deppkg
% 2014-01-23 Foldes: For some reason this was totally broken. Copy_List was renamed Dep_List which is a conflict. Updated to work better too
...
if ~exist('DEST') || isempty(DEST)
    [pathstr, name] = fileparts(which(SRC));
...
if isdir(SRC)
    ...
    for nn = 1:length(File_List)
        if ~File_List(nn).isdir
            ...
            Copy_List =  [Copy_List; dependency_pkg(file_name, DEST, IS_recursive)]; %#ok<AGROW>
        end
    end
    return
end
...
for  nn = 2:length(Dep_List)
    Copy_List = [Copy_List; dependency_pkg(Dep_List{nn}, DEST, true)]; %#ok<AGROW>
end
