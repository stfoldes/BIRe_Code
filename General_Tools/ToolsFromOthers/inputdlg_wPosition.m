function Answer=inputdlg_wPosition(Pos,Prompt, Title, NumLines, DefAns, Resize)
%
% if Pos is empty or 0, will use default figure position - get(0,'DefaultFigurePosition');
% Vertical position not working 2013-08-23
%
...
%
% UPDATES
% 2013-02-07 FOLDES: Will use default figure position if Pos input is empty or 0
...
if ~exist('NumLines')||isempty(NumLines) % FOLDES: Come on MATLAB...
  NumLines=1;
end
...
if isempty(Pos) || max(size(Pos))<2 
    FigPos = get(0,'DefaultFigurePosition');
else
    set(0,'Units','pixels');  
    scnsize=get(0,'ScreenSize');
    % FigPos    = get(0,'DefaultFigurePosition');
    FigPos(1)=Pos(1)*scnsize(3); 
    FigPos(2)=Pos(2)*scnsize(4);  
end
set(InputFig,'Position',FigPos); 

FigPos(3)=FigWidth;
FigPos(4)=FigHeight;
% set(InputFig,'Position',getnicedialoglocation(FigPos,get(InputFig,'Units'))); %********CHANGED*******
...
% setdefaultbutton(InputFig, OKHandle); %*******CHANGED**********
