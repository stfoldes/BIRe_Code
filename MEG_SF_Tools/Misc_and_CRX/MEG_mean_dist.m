function [meanD, varargout] =  MEG_mean_dist(fif_info,fif)
...
%
%   Usage: [meanD, varargout] =  MEG_mean_dist(fif_info,fif)
%   fif_info and fiff are header and data strucutures from read_fiff.m
...
% Intially written by Will
 ...
 for c = 1:length(CHPIs); 
  ...
  CHPIidxs(c) = find( strcmp(fif_info.info.ch_names, CHPIs{c}) );
 end
 ...
 % ensure fif_info.info.chs.coil_trans represents postion: graph it
 ...
 % add to censor cords
 count=0; 
 for i=1:length(fif_info.info.chs)
   trans=fif_info.info.chs(i).coil_trans;
   if(length(trans)>3)
     count=count+1;  
