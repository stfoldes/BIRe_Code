...
%% Go through all files and mark data        
    ifile = 1; redo_flag=0;
    while ifile <= length(metadata_entry_idx)
        ...
        %% Parameters
        current_metadata_entry_idx=metadata_entry_idx(ifile);
        Extract = Prep_Extract_w_Metadata(Extract,Metadata,current_metadata_entry_idx);
        ...
        %% Load EMG,MISC,STI channels
        [EMG_data_raw] = Load_EMG_from_FIF(Extract);
        [processed_EMG_data] = Calc_Processed_EMG(EMG_data_raw,Extract.data_rate);
        
        [MISC_data_raw] = Load_MISC_from_FIF(Extract);
        [processed_MISC_data] = Calc_Processed_EMG(MISC_data_raw,Extract.data_rate);

        [TimeVecs.target_code_org]=Load_TargetCode_from_FIF(Extract);
        ...
        STI_data = TimeVecs.target_code;
        STI_data(STI_data>100)=-1;
        ...
        %% PLOT
        fig=figure;hold all
        plot(zscore(STI_data)+12,'--k','LineWidth',3) % Trigger
        try; plot(zscore(processed_MISC_data(:,7))+8,':','Color','k','LineWidth',3) % Photodiode
        catch; plot(zscore(processed_MISC_data(:,2))+8,':','Color','k','LineWidth',3) % Photodiode
        end
        plot(zscore(processed_MISC_data(:,1))+4,'Color','k','LineWidth',2) % Acc
        plot(zscore(processed_EMG_data(:,1)),'Color',0.5*[1 1 1]) % EMG 1
        plot(zscore(processed_EMG_data(:,2)),'Color',0.5*[1 0 1]) % EMG 2
        ylabel('SDs')
        xlabel('Samples (~S*1000)')
        legend('Trigger','MISC','MISC','EMG','EMG')
        ylim([-2 14]);       
        itrial=1;
