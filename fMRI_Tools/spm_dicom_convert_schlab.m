function out = spm_dicom_convert_schlab(hdr,opts,root_dir,format)
% NOTE, THE BASEFUNCTION WAS UPDATED IN 2011
%
% Custom modification in output handling of file names from the original
% spm_dicom_convert function in SPM8.
%
...
% UPDATE:
% 2012-07-15 MR/SFoldes: Made compatible with PCWIN64
% 2012-08-06 SFoldes: Made compatible with GLNXA64
...
%     switch computer % UPDATE: 2013-01-04 Foldes
...
%         switch computer
%             case {'PCWIN','PCWIN64','GLNXA64'} % UPDATE: 2013-01-04 Foldes
    ...
    if checkfields(hdr,'SeriesNumber','AcquisitionNumber')
        if checkfields(hdr,'EchoNumbers')
            fname = sprintf('%s%s-%.4d-%.5d-%.6d-%.2d.%s', prefix, strip_unwanted(hdr.PatientID),...
                hdr.SeriesNumber,  hdr.InstanceNumber, hdr.AcquisitionNumber,...
