...
[Inverse,Inverse_filename] =        BST_Load_File(Extract,'inverse');
[SurfaceFile] =                     BST_Load_File(Extract,'surface');

%% Implant Sensor Data into AvgFile

[AvgFile,~,AvgFile_fullfilename] =  BST_Load_File(Extract,'avg');
...
if ~exist('hFig') || ~ishandle(hFig)
    ...
    % OverlayFile_str = BST_Build_OverlayFile_str(Extract);
    OverlayFile_str = BST_Build_OverlayFile_str(Extract);
