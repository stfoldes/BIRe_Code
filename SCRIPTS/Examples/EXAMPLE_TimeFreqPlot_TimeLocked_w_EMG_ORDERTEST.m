...
order_list = [8 10 11 12 13 15 16 18 20 25 28 30 32 40 50 60 70 80 100];
clear moddepth_by_band
for iorder=1:length(order_list)
    current_order=order_list(iorder);
    %---Feature Parameters (FeatureParms.)---
    FeatureParms = FeatureParms_Class;
    % Can be empty for loading feature data from CRX files
    FeatureParms.feature_method = 'MEM';
    FeatureParms.order = current_order; %
    FeatureParms.feature_resolution = 1;
    FeatureParms.ideal_freqs = [0:150]; % Pick freq or bins
    FeatureParms.sample_rate = Extract.data_rate;
    FeatureParms.feature_update_rateS = 0.05; % Only needed for timefreq analysis
    ...
    AnalysisParms.post_event_timeS=10;
    ...
    for sensor2plot = sensor2plot_list
        time2plotS = 2.5;%+(AnalysisParms.window_lengthS_move/2); % time related to cue (e.g. 2 = 2s after block start = analysis window that was centered around the first move cue)
...
%         title(['Order: ' num2str(current_order) ', Sensor: ' num2str(sensor2plot)])
%         
%         Figure_Stretch(3,2);
...
%         
        ...
        current_freq_name = 'mu';
        moddepth_by_band.(current_freq_name)(iorder) = mean(mean(moddepth(time2plot,sensor2plot_idx,freq2idx(DEF_freq_bands(current_freq_name),FeatureParms.actual_freqs)),1),3);
        current_freq_name = 'beta';
        moddepth_by_band.(current_freq_name)(iorder) = mean(mean(moddepth(time2plot,sensor2plot_idx,freq2idx(DEF_freq_bands(current_freq_name),FeatureParms.actual_freqs)),1),3);
        current_freq_name = 'SMR';
        moddepth_by_band.(current_freq_name)(iorder) = mean(mean(moddepth(time2plot,sensor2plot_idx,freq2idx(DEF_freq_bands(current_freq_name),FeatureParms.actual_freqs)),1),3);
...
figure;hold all
current_freq_name = 'mu';
plot(order_list,-moddepth_by_band.(current_freq_name),'.-')
current_freq_name = 'beta';
plot(order_list,-moddepth_by_band.(current_freq_name),'.-')
current_freq_name = 'SMR';
plot(order_list,-moddepth_by_band.(current_freq_name),'.-')
current_freq_name = 'gamma';
plot(order_list,moddepth_by_band.(current_freq_name),'.-')
legend({'mu','beta','SMR','gamma'})
xlabel('MEM order')
ylabel('abs(Modulation) (T)')
